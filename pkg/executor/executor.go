...
import (
	...
	"github.com/colonyos/executors/docker/pkg/debug"
	"github.com/colonyos/executors/docker/pkg/docker"
	"github.com/colonyos/executors/docker/pkg/failure"
	"github.com/colonyos/executors/docker/pkg/parsers"
	"github.com/colonyos/executors/docker/pkg/sync"
...
func (e *Executor) createColoniesExecutorWithKey(colonyName string) (*core.Executor, string, string, error) {
	...
	// NOTE: NodeMetadata functionality commented out - requires newer version of colonies library
...
func CreateExecutor(opts ...ExecutorOption) (*Executor, error) {
	...
	randomFunction := &core.Function{ExecutorName: e.executorName, ColonyName: e.colonyName, FuncName: "random"}
	e.client.AddFunction(randomFunction, e.executorPrvKey)
...
func (e *Executor) RandomNumber(min, max int) int {
	if min > max {
		min, max = max, min
	}
...
func (e *Executor) executeRandom(process *core.Process) bool {
	// Parse min and max from kwargs
	minVal := 0
	maxVal := 100

	if minArg, exists := process.FunctionSpec.KwArgs["min"]; exists {
		minStr, ok := minArg.(string)
		if !ok {
			e.failureHandler.HandleError(process, errors.New("'min' argument must be a string"), "Failed to parse 'min' argument")
			return false
		}
		if val, err := strconv.Atoi(minStr); err == nil {
			minVal = val
		} else {
			e.failureHandler.HandleError(process, err, "Failed to parse 'min' argument")
			return false
		}
	}

	if maxArg, exists := process.FunctionSpec.KwArgs["max"]; exists {
		maxStr, ok := maxArg.(string)
		if !ok {
			e.failureHandler.HandleError(process, errors.New("'max' argument must be a string"), "Failed to parse 'max' argument")
			return false
		}
		if val, err := strconv.Atoi(maxStr); err == nil {
			maxVal = val
		} else {
			e.failureHandler.HandleError(process, err, "Failed to parse 'max' argument")
			return false
		}
	}

	// Generate random number
	randomNum := e.RandomNumber(minVal, maxVal)

	// Add result as output attribute
	attr := core.CreateAttribute(process.ID, core.GenerateRandomID(), "", core.OUT, "random", strconv.Itoa(randomNum))
	_, err := e.client.AddAttribute(attr, e.executorPrvKey)
	if err != nil {
		e.failureHandler.HandleError(process, err, "Failed to add output attribute")
		return false
	}

	// Log the result
	logMsg := fmt.Sprintf("Generated random number: %d (min: %d, max: %d)\n", randomNum, minVal, maxVal)
	err = e.client.AddLog(process.ID, logMsg, e.executorPrvKey)
	if err != nil {
		log.WithFields(log.Fields{"Error": err}).Error("Failed to add log")
		return false
	}
...
func (e *Executor) ServeForEver() error {
	for {
		...
		} else if process.FunctionSpec.FuncName == "random" {
			ok := e.executeRandom(process)
			if ok {
				e.client.Close(process.ID, e.executorPrvKey)
			}
		} else {
